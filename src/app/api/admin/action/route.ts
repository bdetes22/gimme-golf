...
async function ensureMemberLogin(customerId: string) {
  try {
...
<body style="margin:0;padding:0;background-color:#060A07;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:40px 24px;">
    ...
    console.error("[ADMIN] ensureMemberLogin failed:", err);
  }
}
...
<body style="margin:0;padding:0;background-color:#060A07;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  ...
  if (action === "add_sessions") {
    ...
    // Make sure this member can actually log in to use their sessions.
  ...
  if (action === "set_membership") {
    ...
    if (type !== "walkin") {
      ...
      // Make sure this member can actually log in to use their membership.
      await ensureMemberLogin(customerId);
    }
